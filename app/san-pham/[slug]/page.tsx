...
export default async function ProductDetailPage({ params }: { params: ParamsLike }) {
  ...
  // IMPORTANT: KHÔNG encodeURI() ở đây để tránh %2520
  ...
  return (
    <main className="max-w-7xl mx-auto px-6 pt-32 pb-16">
      <nav className="mb-6 text-sm text-white/70 flex flex-wrap items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <ProductDetailClient slug={product.slug} name={product.name} defaultImage={product.image} extraImages={extraImages} />
          ...
          <div className="rounded-[32px] border border-white/10 bg-gradient-to-b from-white/10 to-white/5 backdrop-blur-2xl shadow-[0_40px_120px_rgba(0,0,0,0.65)] p-6 md:p-7">
            ...
            {isEdison ? (
              <div className="hidden lg:block">
                <SkuSelector slug={product.slug} defaultImage={product.image} skus={edisonSkus} />
              </div>
            ) : null}
